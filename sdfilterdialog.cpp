...
/*
 * The gradient of the image can be solved by the first derivative and the second partial derivative.
 * But images are stored in the form of a matrix, which cannot be the same as the derivation of straight lines or curves in mathematical theory.
 * Derivation of an image is equivalent to derivation of a plane or surface. For image operations,
 * we use templates to perform convolution operations on the original image to achieve the desired effect.
 * The gradient of an image is transformed into: template (Roberts, Prewitt, Sobel, Lapacian operator) to convolve the original image.
 */
float robertsX[9] = {
     0, 0, 0,
     0,-1, 0,
...
/*
 * Laplace operator is more sensitive to discrete points and noise when operating on images to achieve edge detection.
 * Therefore, first perform Gaussian convolution filtering on the image for noise reduction,
 * and then use Laplace operator for edge detection, which can improve the operator's Robust against noise and discrete points.
 * This process is Laplacian of Gaussian (LOG) filtering.
 */
...
/*
 * Due to the influence of the parameter sigma of the Gaussian function on the Gaussian operator,
 * if the sigma is selected very small, the blurring effect in the early stage is very weak,
 * and it can be approximately equal to the Laplacian operator.
 * Therefore, the Laplacian can also be regarded as a limit case of the Gauss-Laplacian.
 */

/*
 * Use Gaussian function to perform blur operation or convolution operation on the image.
 * The area where it works more obviously is within 3*sigma from the current pixel.
 * It can be seen from this that the larger the sigma,
 * the more neighborhood pixels the Gaussian function can handle, and the more obvious the effect of Gaussian blur.
 */
...
//using Roberts,Prewitt,Sobel or Lapacian operator to filter image
//inputFilterType = Roberts/Sobel/Prewitt/Laplacian4/Laplacian8
...
//Set image for Filter algorithm
...
//Using Filter algorithm to update Image
