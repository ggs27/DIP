...
/*
 * The frequency of an image is an index that characterizes the severity of grayscale changes in the image,
 * and is the gradient of grayscale in a plane space. From the bright and dark bright spots seen on the Fourier spectrogram,
 * in fact, the difference between a point on the image and the neighboring point is the size of the gradient, that is,
 * the size of the frequency of the point (it can be understood that the image in the The low frequency part refers to the point of low gradient,
 * the high frequency part is the opposite). Normally, if the gradient is large, the brightness of the point is strong,
 * otherwise the brightness of the point is weak. This way, by observing the frequency spectrum after the Fourier transform,
 * it is also called the power map. In the power map, we can see the energy distribution of the image.
 * If there are more dark points in the spectrogram, the actual image is softer
 * (because the points are not much different from the neighborhood, and the gradient is relatively small),
 * on the contrary, If there are many bright points in the spectrogram,
 * the actual image must be sharp, with clear boundaries and large differences in pixels on both sides of the boundary.
 */
...
/*
 * After the Fourier transform (fft2), the low-frequency part is concentrated in the four corners, after shifting (fftshift),
 * it is concentrated to the center of the image, this is just for ease of processing
 */
...
//Convert the image from spectrum to QImage
...
/*
 * FFT is not a new transform, it is just an improvement of the Fourier transform algorithm implementation process.
 * The butterfly algorithm is more commonly used in FFT. The butterfly algorithm mainly uses the separability,
 * symmetry and periodicity of the Fourier transform to simplify the amount of DFT calculations.
 */
//1-dimensional Fourier transform operation
...
/*
 * After the inverse Fourier transform (ifft) is performed on the image that has undergone Fourier transform and operation,
 * the time domain image can be restored and the noise on the image can be removed
 */
...
//2-dimensional Fourier transform operation
...
////Convert the image from IFFT image to QImage
...
//2-dimensional fftshift operation
...
/*
 * generate Filter:
 * IdealLowPass:low-pass filter
 * IdealHighPass:high-pass filter
 * GaussainLowPass:low-pass filter
 * ButterworthLowPass:low-pass filter
 * ButterworthHighPass:high-pass filter
 */
...
/*
 * Calculate Image Spectrum
 * Mainly divided into 3 steps
 * 1.Calculate original spectrum
 * 2.fftshift
 * 3.spectrum to QImage
 */
...
/*
 * using fft filter the image
 * Mainly divided into 4 steps
 * 1.filter
 * 2.filtered spectrum
 * 3.fftshift
 * 4.spectrum to QImage
 */
